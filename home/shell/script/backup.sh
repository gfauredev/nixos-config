...
important_dirs="$HOME/life $HOME/project $HOME/.graph" # Always backed up
avail=$(\df --output=avail "$1" | tail -n1)            # Available destination
used=$(\du -c $important_dirs | tail -n1 | cut -f1)    # Used by important dirs
