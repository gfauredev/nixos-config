...
# 3. project.large, same but too large to sync with phone
# 4. data, files that might be interesting in the future
# 4. data.large, same but too large to sync with phone
# 5. archive, finished projects, eventually no more used "life" documents
# life.large does not exits as files are too important to not be synced with phone

avail=$(\df --output=avail "$1"|tail -n1)
used=$(\du -c $HOME/{data,life,project{,.large}}|tail -n1|cut -f1)
...
if [ $avail -gt $used ]; then
  if [[ "$1" == *"back"* ]]; then
    ...
    restic -r "$1" -v backup $HOME/{archive,data{,.large},life,project{,.large}} --exclude ".stversions/" --exclude ".stfolder/" --exclude ".venv*/" --exclude ".vagrant/"
  else
    ...
    rsync --verbose --archive --delete --human-readable --partial --progress \
      ...
      $HOME/{data,life,project{,.large}} "$1"
  fi
else
