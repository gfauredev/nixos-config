    ...
    in
    {
      ...
      config = {
        keybindings = {
          ...
          # Tools
          "${mod}+d" = "exec fullscreen screenshot $HOME/img/$(date +'%Y-%m-%d_%H-%M-%S.png')";
          "${mod}+Shift+d" = "exec --no-startup-id import $HOME/img/$(date +'%Y-%m-%d_%H-%M-%S.png')";
        ...
        startup = [
          ...
          { command = "${term-menu} -e sh -c 'up; veramount'"; }
        ];
